...
class Block {
    static calcBlockHash = (
        ...
        timestamp: number,
        data: string): string =>
        CJs.SHA256(index + previousHash + timestamp + data).toString();
...
const createNewBlock = (data: string): Block => {
    ...
    const newHash: string = Block.calcBlockHash(newIndex, previousBlock.hash, newTimestamp, data);
...
const getHashforBlock = (aBlock: Block): string =>
    Block.calcBlockHash(aBlock.index,
        aBlock.previousHash,
        aBlock.timestamp,
        aBlock.data);
